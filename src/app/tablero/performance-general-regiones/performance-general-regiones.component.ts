...
export class PerformanceGeneralRegionesComponent implements OnInit, OnDestroy {
  ...
  constructor(public userservice: UserService, private serviceAuth: AuthServiceService,
  ...
  ngOnInit(): void {
    ...
    //  this.translateService.use(this.langDefault);
      this.queryPerformanceRegion =this.serviceGraphql.getPerformanceGeneralRegiones(
        this.userData_aux?.idUsuario,this.userData_aux?.companiaId,menuCT.categoriacompaniaid,
        menuCT.idtablero,new Date().getFullYear(),this.getCurrenlyMonth(),this.selectedCoin
      ...
      this.queryLogin = this.serviceGraphql.postLogin(this.serviceAuth.userData?.name,
        ...
        this.queryPerformanceRegion = this.serviceGraphql.getPerformanceGeneralRegiones(
          this.userData_aux?.idUsuario,this.userData_aux?.companiaId,menuCT.categoriacompaniaid,
          menuCT.idtablero,
          filtro.anioActual,filtro.mesActual,filtro.monedaActual
        ).valueChanges.subscribe((response: any) => {
          if (response) {
            this.hideloader();
          }
  ...
  refreshQuery() {
   ...
   // localStorage.removeItem('filtroAMM');
    ...
    if (this.userservice.responseLogin) {
      ...
      this.queryPerformanceRegion = this.serviceGraphql.getPerformanceGeneralRegiones(
        this.userData_aux?.idUsuario,this.userData_aux?.companiaId,menuCT.categoriacompaniaid,
        menuCT.idtablero, Number(this.selectedyear), this.selectedMonth, this.selectedCoin
      ).valueChanges.subscribe((response: any) => {
        if (response) {
          this.hideloader();
        }
