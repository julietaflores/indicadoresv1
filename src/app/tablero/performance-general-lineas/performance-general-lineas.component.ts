...
export class PerformanceGeneralLineasComponent implements OnInit {
  ...
  constructor(public userservice: UserService,
     private serviceAuth: AuthServiceService, private ruta: ActivatedRoute,
    public translate: TranslateService, private route: Router, private serviceGraphql: GraphqlServiceService) {
  ...
  ngOnInit(): void {
    ...
    if (this.userservice.responseLogin) {
      ...
      this.queryPerformanceGL = this.serviceGraphql.getPerformanceGeneralLineas(this.idUsuario_aux,
        this.companiaId_aux, menuCT.categoriacompaniaid, menuCT.idtablero, new Date().getFullYear(),
        ...
        valueChanges.subscribe((response: any) => {
          if (response.data.performancelineal.listames && response.data.performancelineal.listaanual) {
            ...
            this.setSizeCanvas(listaPerformanceMes, listaPerformanceYear);
      ...
      this.queryLogin = this.serviceGraphql.postLogin(this.serviceAuth.userData?.name, this.serviceAuth.userData?.password).
        valueChanges.subscribe((response: any) => {
          ...
          this.queryPerformanceGL = this.serviceGraphql.getPerformanceGeneralLineas(
            this.userData_aux?.idUsuario, this.userData_aux?.companiaId, menuCT.categoriacompaniaid,
            menuCT.idtablero, filtro.anioActual, filtro.mesActual, filtro.monedaActual
          ).valueChanges.subscribe((response: any) => {
            if (response.data.performancelineal.listames && response.data.performancelineal.listaanual) {
  ...
  setSizeCanvas(listaPerformanceMes: any, listaPerformanceYear: any) {
  ...
  refreshQuery() {
    ...
    if (this.userservice.responseLogin) {
      ...
      this.queryPerformanceGL = this.serviceGraphql.getPerformanceGeneralLineas(this.userData_aux?.idUsuario,
        this.userData_aux?.companiaId, menuCT.categoriacompaniaid, menuCT.idtablero, Number(this.selectedyear), this.selectedMonth, this.selectedCoin).
        valueChanges.subscribe((response: any) => {
          if (response.data.performancelineal.listames && response.data.performancelineal.listaanual) {
